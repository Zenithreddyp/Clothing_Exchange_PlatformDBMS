    ...
    <div className="bg-white/5 border border-white/10 rounded-lg p-4 space-y-2">
      <div className="flex items-center justify-between">
        <h4 className="font-semibold">
          {request.requested_title || request.item_title || "Item"}
        </h4>
      ...
      {typeof request.offered_points === "number" &&
        request.offered_points > 0 && (
          <p className="text-white/70 text-sm">
            Points: {request.offered_points}
          </p>
        )}
      {request.requested_image && (
        <img
          src={request.requested_image}
          alt={request.requested_title}
        ...
        <div className="flex gap-2 pt-2">
          {onAccept && (
            <button
              ...
              className="px-3 py-1 rounded bg-white/10 hover:bg-green-500/40 hover:text-white transition-colors duration-200"
            >
              Accept
            </button>
          )}
          {onReject && (
            <button
              ...
              className="px-3 py-1 rounded bg-white/10 hover:bg-red-500/30 hover:text-white transition-colors duration-200"
            >
              Reject
            </button>
          )}
